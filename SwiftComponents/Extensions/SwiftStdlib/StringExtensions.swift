...
public extension String {
    ...
    static func random(ofLength length: Int) -> String {
        guard length > 0 else { return "" }
        let base = "江泽民（1926年8月17日－），男，汉族，江苏扬州人，中国政治家、工程师，中国共产党前中央委员会总书记、中华人民共和国前主要党政官员，是中华人民共和国历史上的第三代最高领导人[1]。江泽民祖籍安徽省宣城旌德县江村（一说江西省婺源江湾镇）[2][3]，1926年8月17日出生于江苏扬州，1946年4月加入中国共产党，毕业于交通大学。江泽民出身于电力机械制造业，在汽车制造业亦有工作领导经历。江泽民早期曾担任过中国电子工业部部长，中共上海市委副书记、市长、市委书记等职务。1989至2002年间担任中国共产党中央委员会总书记，1993至2003年间担任中华人民共和国主席，军队方面于1989至2004年担任中国共产党中央军事委员会主席[4]，1990至2005年担任中华人民共和国中央军事委员会主席。江泽民是中国共产党第十二至十五届中央委员；第十三至十五届中央政治局委员、常委。 中共官方自1989年6月江泽民当选总书记以来，惯常称其为党的第三代中央领导集体的核心[注 1]。其理论著作有“三个代表”重要思想、《江泽民文选》等。夫人王冶坪，两人育有二子。"
        var randomString = ""
        for _ in 1...length {
...
extension String {
    ...
    private var emojiArray: [[String: String]] {
        
        let filePath = "\(Bundle.main.path(forResource: "emojis", ofType: "bundle") ?? "")/emoji.plist"
    ...
    var exp: NSRegularExpression {
        try! NSRegularExpression(pattern: "\\[[a-zA-Z0-9\\u4e00-\\u9fa5]+\\]", options: .caseInsensitive)
    }
    
    /// 将带表情文字的字符串转换为真正的表情
    var toEmojiTest: NSAttributedString {
        ...
        var attrs: [NSAttributedString.Key : Any] = [
            NSAttributedString.Key.font : UIFont.font15,
            NSAttributedString.Key.foregroundColor : UIColor.text1,
            NSAttributedString.Key.paragraphStyle : paraStyle,
            NSAttributedString.Key.baselineOffset : 0
        ]
        let attrText = NSMutableAttributedString(string: "", attributes: attrs)
        
        var index = 0
        let strRange = NSMakeRange(0, count)
        exp.enumerateMatches(in: self, options: .reportProgress, range: strRange) { (result, flags, stop) in
            
            guard let result = result else { return }
            
            // 有表情的情况下需要重新设置偏移：表情使用 15 号文字 12 号
            attrs[NSAttributedString.Key.baselineOffset] = UIFont.font18.ascender - UIFont.font15.ascender
            
            let rangeText = self[result.range.lowerBound..<result.range.upperBound]
            
            if result.range.lowerBound > index {
                
                let rawText = self[index..<result.range.lowerBound]
                attrText.append(NSAttributedString(string: String(rawText), attributes: attrs))
            }
            
            let attach = NSTextAttachment()
            attach.bounds = CGRect(x: 0, y: 0, width: UIFont.font18.ascender, height: UIFont.font18.ascender)
            attach.image = getEmojiImage(with: String(rangeText))
            attrText.append(NSAttributedString(attachment: attach))
            index = result.range.upperBound
        }

        // 有表情的情况下需要重新设置偏移：表情使用 15 号文字 12 号
        if index < strRange.upperBound {
            
            let rawText = self[index..<strRange.upperBound]
            attrText.append(NSAttributedString(string: String(rawText), attributes: attrs))
        }
        ...
        /// 将带表情文字的字符串转换为真正的表情
    var toEmoji: NSAttributedString {
        
        let emojiStrList = self.components(separatedBy: "]")
        let paraStyle = NSMutableParagraphStyle()
        ...
        var attrs: [NSAttributedString.Key : Any] = [
            ...
            NSAttributedString.Key.paragraphStyle : paraStyle,
            NSAttributedString.Key.baselineOffset : 0,
//            NSAttributedString.Key.kern : 1// 字符间距
        ]
        let attrText = NSMutableAttributedString(string: "", attributes: attrs)
        
        for emojiStr in emojiStrList {
            
            let emojiStrs = emojiStr.components(separatedBy: "[")
            if emojiStrs.count == 2 {
                
                // 有表情的情况下需要重新设置偏移：表情使用 15 号文字 12 号
                attrs[NSAttributedString.Key.baselineOffset] = UIFont.font18.ascender - UIFont.font15.ascender
                
                let attrStr = NSAttributedString(string: emojiStrs[0], attributes: attrs)
                attrText.append(attrStr)
                let tag = "[" + emojiStrs[1] + "]"
                let attach = NSTextAttachment()
                attach.bounds = CGRect(x: 0, y: 0, width: UIFont.font18.ascender, height: UIFont.font18.ascender)
                attach.image = getEmojiImage(with: tag)
                attrText.append(NSAttributedString(attachment: attach))
            } else if emojiStrs.count == 1 {
                
                attrText.append(NSAttributedString(string: emojiStrs[0], attributes: attrs))
            }
        }
        return attrText
    }
    
    private func getEmojiImage(with tag: String) -> UIImage? {
        
        for emojiDic in emojiArray {
            
            if emojiDic["tag"] == tag {
                
                guard let fileName = emojiDic["file"] else { return nil }
                
                let filePath = "\(Bundle.main.path(forResource: "emojis", ofType: "bundle") ?? "")/emoji/\(fileName)@2x.png"
